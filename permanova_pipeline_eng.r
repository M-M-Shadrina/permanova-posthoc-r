...
file_path <- readline()
file_path <- gsub('"', '', file_path)  # remove Windows auto-quotes

if (!file.exists(file_path)) stop("File not found. Please check the path.")

cat("\nWhat is the delimiter in the file?\n1 — comma (,)\n2 — semicolon (;)\n")
sep_choice <- readline("Enter 1 or 2: ")
sep_char <- ifelse(sep_choice == "2", ";", ",")

df_raw <- read.csv(file_path, sep = sep_char, stringsAsFactors = FALSE)

cat("\nFile loaded successfully. First rows:\n")
print(head(df_raw))
cat("\nColumns:\n")
print(names(df_raw))
...
cat("\nHow many factors? (2 or 3):\n")
n_factors <- as.integer(readline())
if (!n_factors %in% c(2, 3)) stop("Please enter 2 or 3.")

factors <- c()
for (i in 1:n_factors) {
  cat(sprintf("Enter factor %d name:\n", i))
  f <- trimws(readline())
  if (!f %in% names(df_raw)) stop(sprintf("Column '%s' not found in the table.", f))
  factors <- c(factors, f)
}
...
cat("\nFactors accepted:", paste(factors, collapse = ", "), "\n")

# =========================================================
# STEP 3 — NUMBER OF VARIABLES
# =========================================================

cat("\nHow many variables to analyse?\n")
n_vars <- as.integer(readline())
if (is.na(n_vars) || n_vars < 1) stop("Please enter an integer greater than 0.")

# =========================================================
# STEP 4 — VARIABLE NAMES
# =========================================================

# Author: Maria Shadrina | shadrina@mail.tsu.ru

columns <- c()
for (i in 1:n_vars) {
  cat(sprintf("Enter variable %d name:\n", i))
  v <- trimws(readline())
  if (!v %in% names(df)) stop(sprintf("Column '%s' not found in the table.", v))
  if (v %in% factors) stop(sprintf("'%s' is already used as a factor.", v))
  columns <- c(columns, v)
}
...
if (confirm != "y") stop("Analysis cancelled. Please restart the script.")

# =========================================================
# STEP 6 — OUTPUT PATH AND CORRECTION METHOD
# =========================================================

cat("\nEnter path to save results (press Enter to save to current directory):\n")
...
cat("Results will be saved to:", out_path, "\n")

cat("\nSelect correction method for multiple comparisons (post-hoc):\n")
cat("1 — holm\n2 — bonferroni\n3 — BH\n4 — BY\n5 — none\n")
p_choice <- trimws(readline("Enter number: "))
...
cat("Selected correction method:", p_method, "\n")
...
for (col in columns) {
  ...
  names(tmp)[ncol(tmp)] <- "Value"
  
  if (nrow(tmp) < 12) {
    cat("  [SKIP] Not enough observations:", col, "\n")
    next
  }
  
  # ---------------------------------------------------
  # PRIMARY: adonis2 (PERMANOVA)
  # ---------------------------------------------------
  
  prim <- tryCatch({
    
    d <- dist(tmp$Value, method = "euclidean")
  ...
  # ---------------------------------------------------
  # POST-HOC: pairwise permutation t-test
  # within each level of factor2
  # Author: Maria Shadrina | shadrina@mail.tsu.ru
  # ---------------------------------------------------
  ...
  for (lvl in levels(tmp[[factor2]])) {
    ...
    test <- tryCatch(
      pairwise.perm.t.test(sub$Value, sub[[factor1]],
                           nperm    = 10000,
                           p.method = p_method),
      error = function(e) NULL
    )
    ...
    if (!is.null(test)) {
      ...
      for (i in 1:nrow(pmat)) {
        ...
        d <- safe_d(
          sub$Value[sub[[factor1]] %in% c(r$g1, r$g2)],
...
cat("Primary sig.     :", nrow(primary_sig), "\n")
cat("PostHoc rows     :", nrow(posthoc_all), "\n")
cat("PostHoc sig.     :", nrow(posthoc_sig), "\n")